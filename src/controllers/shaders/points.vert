...
layout(location = 3) in vec2 a_shapeOrigin;
layout(location = 4) in vec3 a_shapeColor;
layout(location = 5) in float a_shapeOpacity;

flat out vec2 v_shapeOrigin;
flat out vec4 v_shapeColor;
...
void main() {
    ...
    v_shapeOrigin = a_shapeOrigin;
    v_shapeColor = vec4(a_shapeColor.rgb, a_shapeOpacity);
}
