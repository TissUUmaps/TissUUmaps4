...
import type { GroupAnnotationsTableColumnDef } from "@/components/widgets/AnnotationsWidget/GroupAnnotationsTable";
...
export function useShapesAnnotationsColumns(
  ...
  const extraGroupColumnDefs: GroupAnnotationsTableColumnDef[] = useMemo(() => {
    ...
    if (
      ...
      if (visibilityMapId !== undefined) {
        ...
        if (visibilityMap !== undefined) {
          groupFillVisibilities = new Map(Object.entries(visibilityMap.values));
        }
      }
      columnDefs.push({
        id: "fillVisibility",
