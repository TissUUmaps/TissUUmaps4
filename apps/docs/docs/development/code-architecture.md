...
`WebGLFrameScheduler` decides when the WebGL renderers draw. After a redraw, it waits for the GPU to finish it and then as long again, so that the overlay takes at most half of the GPU; in between, it moves the drawn pixels into place with a CSS transform. `WebGLShapesRasterizer` constructs scanline data (edge lists and per-bin shape lists) on the CPU for the shapes fragment shader (see [Rendering](./rendering.md)). `SVGController` manages an SVG overlay for interactive shape drawing (rectangle, polygon, and freehand modes).

The package exports only the two contexts, the four renderers, `WebGLFrameScheduler` with its `WebGLRenderer` interface, and `SVGController`; base classes, resolvers, the rasterizer and the `OpenSeadragonUtils`/`WebGLUtils` helper classes are internal. Table rows are looked up by item ID through `TableUtils` from `@tissuumaps/core`, both by the resolvers and by the renderers' per-item layer assignment; the map from ID to row is built at most once per table, and not at all when the IDs to look up are the table's own.
