...
Format metadata belongs in the data provider, not in the tile source. The provider reads the file's channels, colors and pyramid levels itself and hands the tile source explicit images to decode, so that channel count, names, colors and contrast limits come from the file and can be unit-tested here. The TIFF provider works this way.
