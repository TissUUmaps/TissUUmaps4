...
describe("GeometryUtils.intersection", () => {
  it("returns the overlap of two overlapping rectangles", () => {
    const a: Rect = { x: 0, y: 0, width: 10, height: 10 };
    const b: Rect = { x: 5, y: -5, width: 10, height: 10 };
    expect(GeometryUtils.intersection(a, b)).toEqual({
      x: 5,
      y: 0,
      width: 5,
      height: 5,
  ...
  it("returns the contained rectangle when one contains the other", () => {
    const outer: Rect = { x: 0, y: 0, width: 10, height: 10 };
    const inner: Rect = { x: 2, y: 3, width: 4, height: 5 };
    expect(GeometryUtils.intersection(outer, inner)).toEqual(inner);
    expect(GeometryUtils.intersection(inner, outer)).toEqual(inner);
  });

  it("returns null for disjoint rectangles", () => {
    const a: Rect = { x: 0, y: 0, width: 10, height: 10 };
    const b: Rect = { x: 20, y: 20, width: 10, height: 10 };
    expect(GeometryUtils.intersection(a, b)).toBeNull();
  });

  it("returns null for rectangles that only touch", () => {
    const a: Rect = { x: 0, y: 0, width: 10, height: 10 };
...
describe("GeometryUtils.dilate", () => {
  it("moves every side outwards by the amount", () => {
    const rect: Rect = { x: 1, y: 2, width: 3, height: 4 };
    expect(GeometryUtils.dilate(rect, 0.5)).toEqual({
      x: 0.5,
      y: 1.5,
      width: 4,
      height: 5,
  ...
  it("shrinks for a negative amount", () => {
    const rect: Rect = { x: 0, y: 0, width: 10, height: 10 };
    expect(GeometryUtils.dilate(rect, -1)).toEqual({
      x: 1,
      y: 1,
      width: 8,
      height: 8,
...
describe("GeometryUtils.union", () => {
  ...
  it("computes the union of two disjoint rectangles", () => {
    ...
    expect(GeometryUtils.union(a, b)).toEqual({
      x: 0,
      y: 0,
      width: 8,
      height: 6,
  ...
  it("returns the enclosing rectangle when one contains the other", () => {
    ...
    expect(GeometryUtils.union(outer, inner)).toEqual(outer);
    expect(GeometryUtils.union(inner, outer)).toEqual(outer);
  });
  ...
  it("computes the union of more than two rectangles", () => {
    ...
    expect(GeometryUtils.union(a, b, c)).toEqual({
      x: -3,
      y: -2,
      width: 10,
      height: 7,
